 ...
 881                     ; 183 	delay_ms(100);
 883  0159 ae0064        	ldw	x,#100
 884  015c cd000b        	call	_delay_ms
 886                     ; 185 	I2C_start(); /*Accelerometer Configuration*/
 888  015f               LC002:
 889  015f cd004b        	call	_I2C_start
 891                     ; 186 	I2C_address((uint8_t)0x68 << 1, I2C_DIRECTION_TX);
 893  0162 aed000        	ldw	x,#53248
 894  0165 cc007c        	jp	_I2C_address
 938                     ; 192 void uart_init(void){
 939                     	switch	.text
 940  0168               _uart_init:
 942  0168 5208          	subw	sp,#8
 943       00000008      OFST:	set	8
 946                     ; 193 	uint32_t BaudRate_Mantissa = 0, BaudRate_Mantissa100 = 0;
 948  016a 5f            	clrw	x
 949  016b 1f03          	ldw	(OFST-5,sp),x
 950  016d 1f01          	ldw	(OFST-7,sp),x
 953  016f 1f07          	ldw	(OFST-1,sp),x
 954  0171 1f05          	ldw	(OFST-3,sp),x
 955                     ; 195 	UART1->CR1 &= (uint8_t)(~UART1_CR1_M);  /* Clear the word length bit */
 957  0173 72195234      	bres	21044,#4
 958                     ; 196 	UART1->CR1 |= (uint8_t)0x00; /* Set the word length bit, 8 Data bits */
 960  0177 c65234        	ld	a,21044
 961                     ; 197 	UART1->CR3 &= (uint8_t)(~UART1_CR3_STOP);  /* Clear the STOP bits */
 963  017a c65236        	ld	a,21046
 964  017d a4cf          	and	a,#207
 965  017f c75236        	ld	21046,a
 966                     ; 198 	UART1->CR3 |= (uint8_t)0x00; /**< One stop bit is transmitted at the end of frame*/
 968  0182 c65236        	ld	a,21046
 969                     ; 200 	UART1->CR1 &= (uint8_t)(~(UART1_CR1_PCEN | UART1_CR1_PS  ));  /* Clear the Parity Control bit */
 971  0185 c65234        	ld	a,21044
 972  0188 a4f9          	and	a,#249
 973  018a c75234        	ld	21044,a
 974                     ; 201 	UART1->CR1 |= (uint8_t)0x00;  /* Set No Parity Control bit */
 976  018d c65234        	ld	a,21044
 977                     ; 203 	UART1->BRR1 &= (uint8_t)(~UART1_BRR1_DIVM);  /* Clear the LSB mantissa of UART1DIV  */
 979  0190 725f5232      	clr	21042
 980                     ; 204 	UART1->BRR2 &= (uint8_t)(~UART1_BRR2_DIVM);	/* Clear the MSB mantissa of UART1DIV  */
 982  0194 c65233        	ld	a,21043
 983  0197 a40f          	and	a,#15
 984  0199 c75233        	ld	21043,a
 985                     ; 205 	UART1->BRR2 &= (uint8_t)(~UART1_BRR2_DIVF);  /* Clear the Fraction bits of UART1DIV */
 987  019c c65233        	ld	a,21043
 988  019f a4f0          	and	a,#240
 989  01a1 c75233        	ld	21043,a
 990                     ; 212 	UART1->BRR2 = (uint8_t)0x01;
 992  01a4 35015233      	mov	21043,#1
 993                     ; 213 	UART1->BRR1 = (uint8_t)0x34;
 995  01a8 35345232      	mov	21042,#52
 996                     ; 219 	UART1->CR2 |= (uint8_t)UART1_CR2_TEN; /*Transmit Enable and Receive Enable*/ 
 998  01ac 72165235      	bset	21045,#3
 999                     ; 220 	UART1->CR2 |= (uint8_t)UART1_CR2_REN;  
1001  01b0 72145235      	bset	21045,#2
1002                     ; 221 	UART1->CR3 &= (uint8_t)(~UART1_CR3_CKEN); /*Sync mode Disable, SLK pin Disable*/
1004  01b4 72175236      	bres	21046,#3
1005                     ; 222 	UART1->CR1 &= (uint8_t)(~UART1_CR1_UARTD); /* UART1 Enable */
1007                     ; 223 }
...
1069                     ; 225 void uart1_number(int number)
1069                     ; 226 {
1070                     	switch	.text
...
1078                     ; 227     char count = 0;
1080  01c2 0f08          	clr	(OFST+0,sp)
1081                     ; 228     char digit[5] = "";
...
1091  01d3               L304:
1092                     ; 231         digit[count] = number%10;   //lay chu so ngoai cung xxxx8;
...
1201  021f               L744:
1202                     ; 246         UART1->DR = (*cy);
1204  021f c75231        	ld	21041,a
1206  0222               L164:
1207                     ; 247         while((UART1->SR & (uint8_t)UART1_SR_TXE)==RESET){}
...
1223  022f 85            	popw	x
1224  0230 81            	ret	
1247                     ; 251 void uart1_nline(void)
1247                     ; 252 {
1248                     	switch	.text
1249  0231               _uart1_nline:
1253                     ; 253     UART1->DR = (0x0a);
1255  0231 350a5231      	mov	21041,#10
1257  0235               L105:
1258                     ; 254     while((UART1->SR & (uint8_t)UART1_SR_TXE)==RESET);
